...
async function sendResetLink() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            msgDiv.innerHTML = '<span class="error">' + (data.error || 'Request failed') + '</span>';
        }
    } catch(err) {
        console.error(err);
...
async function submitResetPassword() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            msgDiv.innerHTML = '<span class="error">' + (data.error || 'Reset failed') + '</span>';
        }
    } catch(err) {
        console.error(err);
...
async function previewFile(id, name, type) {
    ...
    try {
        ...
        if (type.startsWith('image/')) {
            ...
            content.innerHTML = `<p style="color:white;">Preview not available. <button class="btn-primary" onclick="downloadFile(${id})">Download</button></p>`;
        }
